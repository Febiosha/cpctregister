...
(async () => {
  ...
  const answer = await askQuestion(chalk.yellow('Masukkan jumlah akun yang ingin dibuat: '));
  const loopCount = parseInt(answer) || 1;
  rl.close();

  console.log(chalk.yellow(`\nMemulai pembuatan ${loopCount} akun...`));

  for (let i = 1; i <= loopCount; i++) {
    try {
      console.log(chalk.cyan(`\n[Akun ${i}/${loopCount}]`));
      console.log(chalk.blue('Membuat email acak...'));
      
      const domainList = await getEmailRandom();
      const domain = domainList[Math.floor(Math.random() * domainList.length)];
      const name = faker.internet.username().toLowerCase().replace(/[^a-z0-9]/g, '');
      const email = `${name}@${domain}`;
      console.log(chalk.green('Email dibuat:'), email);
      ...
      if (reqnya.message === "success") {
        console.log(chalk.blue('Menunggu email verifikasi...'));
        ...
        console.log(chalk.green('Kode verifikasi:'), verificationCode);
        ...
        if (verifyData.message === "success") {
          ...
          console.log(chalk.green('Akun berhasil disimpan!'));
        }
      } else {
        console.log(chalk.red('Gagal membuat akun'));
      }
    } catch (error) {
      console.log(chalk.red(`Error pada iterasi ${i}:`), error.message);
    }
  }

  console.log(chalk.green('\nSemua akun berhasil dibuat!'));
  console.log(chalk.blue(`Data akun tersimpan di: ${path.resolve('accounts.txt')}`));
})();